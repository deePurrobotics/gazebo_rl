...
class CribTaskEnv(TurtlebotRobotEnv):
  ...
  def _set_init(self):
    ...
    goal_state.model_name="flag"
    goal_state.pose = goal_pose
    goal_state.reference_frame='world'
    #f = open('/home/liu/flag.sdf','r') #set your own root where the flag.sdf is
    #sdff = f.read()

    rospy.wait_for_service('gazebo/set_model_state')
    set_model_state_prox = rospy.ServiceProxy('gazebo/set_model_state', SetModelState)
    #spawn_model_prox("some_robo_name", sdff, "robotos_name_space", goal_pose, "world")
    set_model_state_prox(goal_state)
